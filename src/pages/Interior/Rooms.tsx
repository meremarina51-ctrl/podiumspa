    ...
    return (
        <section className="mx-auto w-full max-w-340 px-6 py-16 md:px-14 md:py-24">
            ...
            <div className="grid grid-cols-1 gap-x-5 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
                {ROOMS.map(({ name, bio, photo }, index) => (
                    <div key={index} className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-1 flex-col gap-3">
                            ...
                            <button
                                ...
                                className="mt-auto cursor-pointer inline-flex w-fit items-center justify-center rounded-full bg-accent px-6 py-3 text-sm font-semibold text-white transition-colors duration-200 hover:bg-accent-dark"
                            >
                                {tCommon("orderCta")}
                            </button>
                        </div>
            ...
            <OrderModal
                title={openRoom !== null ? pick(ROOMS[openRoom].name, locale) : ""}
                open={openRoom !== null}
