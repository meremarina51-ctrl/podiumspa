...
export const Feed = () => {
    ...
    const filtered = activeCategory ? posts.filter((post) => post.category.ru === activeCategory) : posts

    return (
        <section className="mx-auto w-full max-w-340 px-6 pb-16 md:px-14 md:pb-24">
            <Link href={`${ROUTES.BLOG}/${featured.index}`} className="group mb-14 block sm:mb-16">
            ...
            <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1fr_300px]">
                            ...
                            >
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <p className="mb-3 text-[13.5px] leading-relaxed text-foreground-muted">
                                        {truncate(pick(post.excerpt, locale), 150)}
                                    </p>
                ...
                <aside className="flex flex-col gap-8">
                    <div className="rounded-sm border border-border p-6">
                        <p className="mb-4 text-[11px] font-semibold tracking-widest text-foreground-faint uppercase">{t("categories")}</p>
                        <div className="flex flex-col gap-1">
                            <button
                                type="button"
                                onClick={() => setActiveCategory(null)}
                                className={`rounded-sm cursor-pointer px-3 py-2 text-left text-[13.5px] font-medium transition-colors duration-200 ${activeCategory === null
                            ...
                            {CATEGORIES.map((category) => (
                                <button
                                    key={category.ru}
                                    type="button"
                                    onClick={() => setActiveCategory(category.ru)}
                                    className={`rounded-sm cursor-pointer px-3 py-2 text-left text-[13.5px] font-medium transition-colors duration-200 ${activeCategory === category.ru
                                        ? "bg-accent-wash text-accent-light"
                                        : "text-foreground-muted hover:bg-foreground/5 hover:text-foreground"
                                        }`}
                                >
